...
class MesaPage(Gtk.Box):
    ...
    def __init__(self):
        ...
        # Set up initial loading view
        self._setup_loading_view()

        # Schedule content loading after UI is shown
        GLib.idle_add(self._initialize_content)

    def _setup_loading_view(self):
        """Create initial loading view."""
        ...
        loading_label.set_markup(
            "<span size='large'>Loading driver information...</span>"
        )
    ...
    def _initialize_content(self):
        """Initialize content after showing loading spinner."""
        ...
        # Load available Mesa drivers in a separate thread
        GLib.timeout_add(100, self._async_load_drivers)

        return False  # Don't call again

    def _async_load_drivers(self):
        """Load drivers asynchronously."""
        # Show a temporary spinner in the driver box
        temp_spinner = Gtk.Spinner()
        temp_spinner.set_size_request(24, 24)
        temp_spinner.set_valign(Gtk.Align.CENTER)
        temp_spinner.set_halign(Gtk.Align.CENTER)
        temp_spinner.set_margin_top(48)
        temp_spinner.set_margin_bottom(48)
        temp_spinner.set_vexpand(True)
        temp_spinner.start()

        # Add temporary spinner to the list
        self.driver_box.append(temp_spinner)

        # Schedule actual driver loading in a separate thread using Python's threading module
        threading_thread = threading.Thread(target=self._background_load_drivers)
        threading_thread.daemon = True
        threading_thread.start()

        return False  # Don't call again

    def _background_load_drivers(self):
        """Load drivers in background thread."""
        # Get available drivers
        drivers = self.mesa_manager.get_available_drivers()

        # Update UI in main thread
        GLib.idle_add(self._finish_driver_loading, drivers)

        return False

    def _finish_driver_loading(self, drivers):
        """Finish driver loading in the main thread."""
        # Clear existing items including temporary spinner
        while True:
        ...
        # Enable the apply button after loading
    ...
    def _create_content(self):
        ...
        # Disable apply button initially until drivers are loaded
