...
class KernelManager:
    ...
    def install_kernel(self, kernel, progress_callback=None, output_callback=None, complete_callback=None):
        """
    ...
    def _install_kernel_thread(self, packages, progress_callback, output_callback, complete_callback):
        """
        ...
        if progress_callback:
            progress_callback(0.1, f"Starting installation of {packages[0]} and modules...")
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ""):
                ...
                if "downloading" in line.lower() or "baixando" in line.lower() or "download" in line.lower():
                    downloading = True
                    ...
                    if match:
                        ...
                        progress = 0.1 + (current / total) * 0.4  # 10%-50% for downloading
                ...
                elif installing and ("installed" in line.lower() or "instalado" in line.lower()):
                ...
                elif "generating grub configuration file" in line.lower() or "gerando arquivo de configuração do grub" in line.lower():
                    if progress_callback:
                        progress_callback(0.9, "Updating bootloader...")
                
                # Additional progress indicators
                elif "synchronizing package databases" in line.lower() or "sincronizando bases de dados de pacotes" in line.lower():
                    if progress_callback:
                        progress_callback(0.1, "Synchronizing package databases...")
                
                elif "checking dependencies" in line.lower() or "verificando dependências" in line.lower():
                    if progress_callback:
                        progress_callback(0.2, "Checking dependencies...")
                
                elif "checking for file conflicts" in line.lower() or "verificando conflitos de arquivos" in line.lower():
                    if progress_callback:
                        progress_callback(0.4, "Checking for file conflicts...")
                        
                # Package sizes and totals  
                elif "total download size" in line.lower() or "tamanho total de download" in line.lower():
                    if output_callback:
                ...
                if pkg_match and installing:
                    ...
                    if progress_callback:
                        progress_callback(progress, f"Installing {pkg_name} {pkg_version}")
                ...
                if current_time - last_line_time > 5:
                    if output_callback:
                        output_callback(f"Still working... (last action: {progress:.0%} complete)")
            ...
            if output_callback:
                if success:
                    ...
                    output_callback(f"❌ Installation failed with return code {process.returncode}.")
    ...
    def remove_kernel(self, kernel, progress_callback=None, output_callback=None, complete_callback=None):
        """
        ...
        threading.Thread(
            ...
            args=(installed_packages, progress_callback, output_callback, complete_callback),
            daemon=True
        ).start()
    
    def _remove_kernel_thread(self, packages, progress_callback, output_callback, complete_callback):
        """
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ""):
                ...
                if "checking dependencies" in line.lower() or "verificando dependências" in line.lower():
                    progress = 0.2
                    removal_step = 1
                    if progress_callback:
                        progress_callback(progress, "Checking dependencies...")
                        
                elif "looking for conflicting packages" in line.lower() or "procurando por pacotes conflitantes" in line.lower():
                    progress = 0.3
                    removal_step = 2
                    if progress_callback:
                        progress_callback(progress, "Looking for conflicting packages...")
                    ...
                    if total_packages > 0:
                        ...
                        progress = 0.5  # Default progress if we can't calculate accurately
                    removal_step = 3
                    if progress_callback:
                        progress_callback(progress, f"Removing packages ({packages_processed}/{total_packages})...")
                    
                elif "running post-transaction hooks" in line.lower() or "executando hooks pós-transação" in line.lower():
                    progress = 0.8
                    removal_step = 4
                    if progress_callback:
                ...
                # Calculate progress based on line content if not specifically detected
                    ...
                    progress = 0.85
                    if progress_callback:
                        progress_callback(progress, "Updating bootloader information...")
                ...
                if current_time - last_line_time > 5:
                    if output_callback:
                        output_callback(f"Still working... (removal step {removal_step}, {progress:.0%} complete)")
                    last_line_time = current_time
            ...
            if output_callback:
                if success:
                    ...
                    output_callback(f"Removal failed with return code {process.returncode}.")
